import { USE_CASES, type SignalAggregate, type SignalEstimate } from '@private-signal-swarm/types';
import { z } from 'zod';
...
export function aggregateSignal(roundId: string, estimates: SignalEstimate[]): SignalAggregate {
  const values = estimates.map((e) => e.value);
  const largeCohort = values.length >= 5;
  const output: SignalAggregate = {
...
export const signalEstimateModule: UseCaseModule<SignalEstimate> = {
